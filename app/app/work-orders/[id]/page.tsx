import { getWorkOrder, listQuotes, listScheduleEntries, listTimeEntries, getActiveEmployees, isTechAssignedToWorkOrder } from '@/lib/data'
...
import { WorkOrderSchedulePanel } from '@/components/work-order-schedule-panel'
import { WorkOrderTimePanel } from '@/components/work-order-time-panel'
import { WorkOrderCompletionPanel } from '@/components/work-order-completion-panel'
...
export default async function WorkOrderDetailPage({ params }: { params: { id: string } }) {
  ...
  if (user?.role === 'TECH') {
    ...
    const isAssigned = await isTechAssignedToWorkOrder(params.id, user.employee.id)
    if (!isAssigned) {
      notFound()
    }
  }
  ...
  const scheduleEntries = await listScheduleEntries({ work_order_id: params.id }).catch(() => [])
    ...
    <div>
      ...
      <Tabs defaultValue="overview" className="space-y-6">
        ...
        <TabsContent value="schedule">
          <WorkOrderSchedulePanel
            workOrderId={params.id}
            assignedTo={workOrder.assigned_to}
            scheduleEntries={scheduleEntries}
            employees={employees}
            canEdit={hasPermission(user?.role, 'edit_schedule')}
          />
        </TabsContent>

        <TabsContent value="notes">
          {/* TODO(schema): add work_order_notes table to support execution notes. */}
          <div className="rounded-lg border border-dashed border-slate-200 p-6 text-center text-sm text-slate-500">
            Notes are not yet available for work orders.
          </div>
        </TabsContent>

        <TabsContent value="files">
          {/* TODO(schema): add 'work_order' to file_entity_type for execution files. */}
          <div className="rounded-lg border border-dashed border-slate-200 p-6 text-center text-sm text-slate-500">
            File uploads are not yet available for work orders.
          </div>
        </TabsContent>
        ...
        <TabsContent value="time">
          <WorkOrderTimePanel
            ...
            employees={employees}
            canEdit={hasPermission(user?.role, 'edit_time')}
            currentTechId={user?.role === 'TECH' ? user.employee?.id : null}
          />
        </TabsContent>

        <TabsContent value="completion">
          <WorkOrderCompletionPanel
            workOrderId={params.id}
            status={workOrder.status}
            completedAt={workOrder.completed_at}
            canComplete={hasPermission(user?.role, 'edit_work_orders')}
          />
        </TabsContent>
